...
var url;
	...
	function openSpchongzhiAddDialog(){
	...
	function deleteSpchongzhi(){
		...
		for(var i=0;i<selectedRows.length;i++){
			strIds.push(selectedRows[i].spchongzhiId);
		}
		var ids=strIds.join(",");
		$.messager.confirm("系统提示","您确认要删掉这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
			if(r){
				$.post("../deleteSpchongzhi",{delIds:ids},function(result){
					if(result.success){
						$.messager.alert("系统提示","您已成功删除<font color=red>"+result.delNums+"</font>条数据！");
						$("#dg").datagrid("reload");
					}else{
						$.messager.alert('系统提示','<font color=red>'+selectedRows[result.errorIndex].spchongzhiName+'</font>'+result.errorMsg);
					}
				},"json");
			}
		});
	...
	function closeSpchongzhiDialog(){
		$("#dlg").dialog("close");
		resetValue();
	}
	
	function openSpchongzhiModifyDialog(){
		var selectedRows=$("#dg").datagrid('getSelections');
		...
		$("#fm").form("load",row);
		url="../addSpchongzhi?spchongzhiId="+row.spchongzhiId;
	}
	...
	function daochuSpchongzhi(){
		...
		var spchongzhiIds=[];
		for(var i=0;i<selectedRows.length;i++){
			spchongzhiIds.push(selectedRows[i].spchongzhiId);
		}
		var ids=spchongzhiIds.join(",");
		$.messager.confirm("系统提示","您确认要导出数据吗？",function(r){
			if(r){
				$.post("../daochuSpchongzhi",{delIds:ids},function(result){
					if(result.success){
	...
	function daoruSpchongzhis(){
		$("#daoru").dialog("open").dialog("setTitle","导入用户充值");
		daoruurl="../daoruSpchongzhi";
	}
	
	function closeDaoruSpchongzhi(){
		$("#daoru").dialog("close");
		resetValue();
	}
	
	function saveDaoruSpchongzhi(){
		$("#drfm").form("submit",{
			url:daoruurl,
			onSubmit:function(){
				return $(this).form("validate");
	...
	function shangchuanSpchongzhi(){
		...
		shchurl="../shangchuanSpchongzhi?spchongzhiId="+row.spchongzhiId;
	}
	
	function closeShangchuanSpchongzhi(){
		$("#shangchuan").dialog("close");
		resetValue();
	}
	
	function saveShangchuanSpchongzhi(){
		$("#shchfm").form("submit",{
			url:shchurl,
			onSubmit:function(){
				return $(this).form("validate");
...
<!--startprint-->
	<table id="dg" title="用户充值" class="easyui-datagrid" fitColumns="true"
	 pagination="true" url="../getSpchongzhis" fit="true" toolbar="#tb">
		<thead>
			<tr>
				<th field="cb" checkbox="true"></th>
				<th field="spchongzhiId" width="20">编号</th>
				<th field="yonghuId" width="20" hidden="true">用户ID</th>
				<th field="yonghuName" width="40">用户</th>
				<th field="spchongzhiJine" width="20">金额</th>
				<th field="spchongzhiDate" width="80" formatter="datetostr">时间</th>
			</tr>
		</thead>
	</table>
	...
	<div id="tb" hidden="true">
		<div>
			<a href="javascript:openSpchongzhiAddDialog()" class="easyui-linkbutton" iconCls="icon-edit" plain="true">充值</a>
		</div>
		<div>
		...
		<a href="javascript:searchSpchongzhi()" class="easyui-linkbutton" iconCls="icon-search" plain="true">搜索</a>
		</div>
	</div>
<!--endprint-->
		...
		<form id="fm" method="post">
			<table>
				<tr>
					...
					<td>金额：</td>
					<td><input type="text" name="spchongzhiJine" id="spchongzhiJine" class="easyui-validatebox" required="true"/></td>
				</tr>
			</table>
		</form>
	</div>
	...
	<div id="dlg-buttons">
		<a href="javascript:saveSpchongzhi()" class="easyui-linkbutton" iconCls="icon-ok">保存</a>
		<a href="javascript:closeSpchongzhiDialog()" class="easyui-linkbutton" iconCls="icon-cancel">关闭</a>
	</div>
<!--上传-->	
	...
	<div id="shangchuan-buttons">
		<a href="javascript:saveShangchuanSpchongzhi()" class="easyui-linkbutton" iconCls="icon-ok">保存</a>
		<a href="javascript:closeShangchuanSpchongzhi()" class="easyui-linkbutton" iconCls="icon-cancel">关闭</a>
	</div>
<!--导入-->	
	...
	<div id="daoru-buttons">
		<a href="javascript:saveDaoruSpchongzhi()" class="easyui-linkbutton" iconCls="icon-ok">保存</a>
		<a href="javascript:closeDaoruSpchongzhi()" class="easyui-linkbutton" iconCls="icon-cancel">关闭</a>
	</div>
